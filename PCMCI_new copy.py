    ...
    for each in dir_names:
        try:
            ...
            if data_points_number < 50:
                ...
                dataset_dict_sub50[each]["truemat"] = np.load(str(DATA_PATH+each+"/"+each+"_split_truemat.npy"))
                dataset_dict_sub50[each]["causaldb"] = causal
                for import_type in TO_IMPORT:
                    dataset_dict_sub50[each][import_type] = np.load(str(DATA_PATH+each+"/"+each+"_"+import_type+".npy"))
            elif data_points_number >= 50 and data_points_number < 100:
                dataset_dict_sub100[each] = {}
                dataset_dict_sub100[each]["truemat"] = np.load(str(DATA_PATH+each+"/"+each+"_split_truemat.npy"))
                dataset_dict_sub100[each]["causaldb"] = causal
                for import_type in TO_IMPORT:
                    dataset_dict_sub100[each][import_type] = np.load(str(DATA_PATH+each+"/"+each+"_"+import_type+".npy"))
            elif data_points_number >= 100 and data_points_number < 200:
                dataset_dict_sub200[each] = {}
                dataset_dict_sub200[each]["truemat"] = np.load(str(DATA_PATH+each+"/"+each+"_split_truemat.npy"))
                dataset_dict_sub200[each]["causaldb"] = causal
                for import_type in TO_IMPORT:
                    dataset_dict_sub200[each][import_type] = np.load(str(DATA_PATH+each+"/"+each+"_"+import_type+".npy"))
            elif data_points_number >= 200:
                dataset_dict_over200[each] = {}
                dataset_dict_over200[each]["truemat"] = np.load(str(DATA_PATH+each+"/"+each+"_split_truemat.npy"))
                dataset_dict_over200[each]["causaldb"] = causal
                for import_type in TO_IMPORT:
                    dataset_dict_over200[each][import_type] = np.load(str(DATA_PATH+each+"/"+each+"_"+import_type+".npy"))
        except:
            continue
...
def main():
    ...
    dir_names = []
    get_names()
    dataset_dict = import_data(dir_names)
